func void startup_AI_Testlevel ()
{
    B_InitMonsterAttitudes ();
	B_InitGuildAttitudes();

//	Wld_InsertNpc				(ATestmodell,"WP_HALLYWAY_1");
//	Wld_InsertNpc				(BTestmodell,"WP_CHAIRSNBEDS");

//	Wld_InsertNpc				(friendly1_testmodell,"WP_BSANVIL");
//	Wld_InsertNpc				(friendly2_testmodell,"WP_BSFIRE");
	

/*	Wld_InsertNpc				(CTestmodell,"WP_HALLWAY_2");
	Wld_InsertNpc				(DTestmodell,"WP_STEAL_1");
	Wld_InsertNpc				(ETestmodell,"WP_STEALWITNESS_1");
	Wld_InsertNpc				(FTestmodell,"WP_STEALWITNESS_2");
	Wld_InsertNpc				(GTestmodell,"WP_BSCOOL");
	Wld_InsertNpc				(HTestmodell,"WP_BSSHARP");
	Wld_InsertNpc				(ITestmodell,"WP_BSANVIL");
	Wld_InsertNpc				(JTestmodell,"WP_BSFIRE");*/

/*
	Wld_InsertNpc				(BloodFly,"WP_Scorpion");
	Wld_InsertNpc				(SCAVENGER,"WP_CAVE_9");
	Wld_InsertNpc				(MinecrawlerQUEEN,"WP_CRWQUEEN");
	Wld_InsertNpc				(DEMON,"WP_DEMON");
	Wld_InsertNpc				(GOBBO,"WP_GOBBO");
	Wld_InsertNpc				(GOLEM,"WP_GOLEM");
	Wld_InsertNpc				(LURKER,"WP_LURKER");
	Wld_InsertNpc				(MEATBUG,"WP_MEATBUG");
	Wld_InsertNpc				(MINECRAWLER,"WP_CRAWLER");
	Wld_InsertNpc				(MOLERAT,"WP_MOLERAT");
	Wld_InsertNpc				(Orc_Shaman, "WP_ORCDOG");
	Wld_InsertNpc				(UndeadOrc_Shaman, "WP_ORC");
	Wld_InsertNpc				(ORCDOG, "WP_ORCDOG");
	Wld_InsertNpc				(SHADOWBEAST, "WP_SHADOW");
	Wld_InsertNpc				(SKELETON, "WP_SKELETON");
	Wld_InsertNpc				(SNAPPER, "WP_SNAPPER");
	Wld_InsertNpc				(SWAMPSHARK, "WP_SWAMPSHARK");	
	Wld_InsertNpc				(WARAN, "WP_WARAN");
	Wld_InsertNpc				(TROLL, "WP_TROLL");
	Wld_InsertNpc				(WOLF, "WP_WOLF");
	Wld_InsertNpc				(ZOMBIE, "WP_ZOMBIE");
	Wld_InsertNpc				(SLD1_Testmodell,"WP_CAVE_4");
*/

};


func void startup_SPACER_CHECKLIST_OUTDOOR ()
{
    B_InitMonsterAttitudes ();
	B_InitGuildAttitudes();
};