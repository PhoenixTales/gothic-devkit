...
def register(): 
	register_importer("KrxMshImp", "MSH", "Kerrax Compiled Mesh") 

def unregister(): 
	unregister_importer("KrxMshImp") 

if __name__ == "main": 
	register() 

def KrxMshImp(filename_param, quiet_param = False): 
	QUIET = quiet_param 
	IMPORT_FILE_NAME = filename_param 
	IMPORT_FILE_SIZE = get_file_size(IMPORT_FILE_NAME)
	
	dlgInput = (NewMeshImporterDlgInput())
	dlgInput.SetFileFormat("MSH")
	...
	if( not (QUIET)):
		try:
			...
			inputFile.Close()
			outputFile.Close()
			show_error_box("Kerrax MSH Importer", (ex.args[0] if (len(ex.args) >= 1 and type(ex.args[0]) == str) else str(ex.args)))
			hide_progress_bar()
			return (2)

	loader = (NewMSHFileLoader())
	try:
		if(dlgOutput.GetCurrentSceneMode() == 1):
			reset_scene()

		spaceTransform = (dlgOutput.GetSpaceTransform())
		loader.ReadMSHFile(IMPORT_FILE_NAME, spaceTransform)
