...
class TMRMFileLoader:
	...
	def __ReadVertices(self):
		...
		self. __curMesh.set_num_verts(self. __numVerts)
		self. __file.SetPos(self. __posVerts)
	...
	def __CountTriangles(self):
		self. __numTriangles = (0)
		for i in range((0), (self. __numSubMeshes)):
			self. __file.SetPos(self. __posSubMeshes + i * 80 + 4)
			numTri_subMesh = (self. __file.ReadUnsignedLong())
			self. __numTriangles = (self. __numTriangles + numTri_subMesh)

	def __CountWedges(self):
		self. __numWedges = (0)
		for i in range((0), (self. __numSubMeshes)):
			self. __file.SetPos(self. __posSubMeshes + i * 80 + 12)
			numWdg_subMesh = (self. __file.ReadUnsignedLong())
			self. __numWedges = (self. __numWedges + numWdg_subMesh)

	def __ReadUVMapping(self):
		caption = (FormatMsg1("Reading '%1'", self. __curNameInFile))
		self. __curMesh.set_num_tverts(self. __numWedges)
		tVertIndex = (0)
		for i in range((0), (self. __numSubMeshes)):
			self. __file.SetPos(self. __posSubMeshes + i * 80 + 8)
			posWdg_subMesh = (self. __file.ReadUnsignedLong() + self. __dataPos)
			numWdg_subMesh = (self. __file.ReadUnsignedLong())
			for j in range((0), (numWdg_subMesh)):
				if(((tVertIndex) & (63)) == 0):
					show_progress_bar(caption, 25 + 25 * tVertIndex / self. __numWedges)
				
				self. __file.SetPos(posWdg_subMesh + j * 24 + 12)
				u = (self. __file.ReadFloat())
				v = ( -self. __file.ReadFloat())
				uvvert = (new_uvvert(u, v))
				self. __curMesh.set_tvert(tVertIndex, uvvert)
				tVertIndex = (tVertIndex + 1)

	def __ReadFaces(self):
		self. __curMesh.set_num_faces(self. __numTriangles)
		...
		for i in range((0), (self. __numSubMeshes)):
			self. __file.SetPos(self. __posSubMeshes + i * 80 + 0)
			posTri_subMesh = (self. __file.ReadUnsignedLong() + self. __dataPos)
			numTri_subMesh = (self. __file.ReadUnsignedLong())
			posWdg_subMesh = (self. __file.ReadUnsignedLong() + self. __dataPos)
			numWdg_subMesh = (self. __file.ReadUnsignedLong())
			for j in range((0), (numTri_subMesh)):
				if(((faceIndex) & (63)) == 0):
					show_progress_bar(caption, 50 + 50 * faceIndex / self. __numTriangles)

				self. __file.SetPos(posTri_subMesh + j * 6 + 0)
				wdgIdx_0 = (self. __file.ReadUnsignedShort())
				self. __file.SetPos(posWdg_subMesh + wdgIdx_0 * 24 + 20)
				vertIdx_0 = (self. __file.ReadUnsignedShort())
				tVertIdx_0 = (wdgIdx_0 + tVertBase)
				
				self. __file.SetPos(posTri_subMesh + j * 6 + 2)
				wdgIdx_1 = (self. __file.ReadUnsignedShort())
				self. __file.SetPos(posWdg_subMesh + wdgIdx_1 * 24 + 20)
				vertIdx_1 = (self. __file.ReadUnsignedShort())
				tVertIdx_1 = (wdgIdx_1 + tVertBase)
				
				self. __file.SetPos(posTri_subMesh + j * 6 + 4)
				wdgIdx_2 = (self. __file.ReadUnsignedShort())
				self. __file.SetPos(posWdg_subMesh + wdgIdx_2 * 24 + 20)
				vertIdx_2 = (self. __file.ReadUnsignedShort())
				tVertIdx_2 = (wdgIdx_2 + tVertBase)
				
				if(vertIdx_0 >= numVerts):
					raise RuntimeError(FormatMsg4("Vertex index is out of range while reading multi-resolution mesh.\nVertex index: %1 (Allowable range: %2..%3).\nFile name: \"%4\".", (int_to_string(vertIdx_0)), "0", (int_to_string(numVerts - 1)), self. __file.GetName()))
				
				if(vertIdx_1 >= numVerts):
					raise RuntimeError(FormatMsg4("Vertex index is out of range while reading multi-resolution mesh.\nVertex index: %1 (Allowable range: %2..%3).\nFile name: \"%4\".", (int_to_string(vertIdx_1)), "0", (int_to_string(numVerts - 1)), self. __file.GetName()))
				
				if(vertIdx_2 >= numVerts):
					raise RuntimeError(FormatMsg4("Vertex index is out of range while reading multi-resolution mesh.\nVertex index: %1 (Allowable range: %2..%3).\nFile name: \"%4\".", (int_to_string(vertIdx_2)), "0", (int_to_string(numVerts - 1)), self. __file.GetName()))
				
				if(tVertIdx_0 >= numTVerts):
					raise RuntimeError(FormatMsg4("Texture vertex index is out of range while reading multi-resolution mesh.\nTexture vertex index: %1 (Allowable range: %2..%3).\nFile name: \"%4\".", (int_to_string(tVertIdx_0)), "0", (int_to_string(numTVerts - 1)), self. __file.GetName()))
				
				if(tVertIdx_1 >= numTVerts):
					raise RuntimeError(FormatMsg4("Texture vertex index is out of range while reading multi-resolution mesh.\nTexture vertex index: %1 (Allowable range: %2..%3).\nFile name: \"%4\".", (int_to_string(tVertIdx_1)), "0", (int_to_string(numTVerts - 1)), self. __file.GetName()))
				
				if(tVertIdx_2 >= numTVerts):
					raise RuntimeError(FormatMsg4("Texture vertex index is out of range while reading multi-resolution mesh.\nTexture vertex index: %1 (Allowable range: %2..%3).\nFile name: \"%4\".", (int_to_string(tVertIdx_2)), "0", (int_to_string(numTVerts - 1)), self. __file.GetName()))

				f = (new_face(vertIdx_0, vertIdx_1, vertIdx_2))
				self. __curMesh.set_face(faceIndex, f)
				tf = (new_tvface(tVertIdx_0, tVertIdx_1, tVertIdx_2))
				self. __curMesh.set_tvface(faceIndex, tf)
				self. __curMesh.set_face_material(faceIndex, ((self. __impMtls)[i]))
				for k in range((0), (3)):
					pass
				
				faceIndex = (faceIndex + 1)
			
			tVertBase = (tVertBase + numWdg_subMesh)

	def __ReadMaterials(self):
		self. __file.SetPos(self. __posMaterials)
		zarc = (NewZENArchive())
	...
	def __ReadMrmData(self):
		self. __mrmVersion = (self. __file.ReadUnsignedShort())
		self. __dataSize = (self. __file.ReadUnsignedLong())
		self. __dataPos = (self. __file.GetPos())
		self. __file.SetPos(self. __dataPos + self. __dataSize)
		self. __numSubMeshes = (self. __file.ReadUnsignedChar())
		
		self. __posVerts = (self. __file.ReadUnsignedLong() + self. __dataPos)
		self. __numVerts = (self. __file.ReadUnsignedLong())
		posTAxes = (self. __file.ReadUnsignedLong() + self. __dataPos)
		numTAxes = (self. __file.ReadUnsignedLong())
		self. __posSubMeshes = (self. __file.GetPos())
		self. __posMaterials = (self. __posSubMeshes + 80 * self. __numSubMeshes)
		
		self. __ReadMaterials()
		self. __CreateObject()
		self. __ReadVertices()
		self. __CountTriangles()
		self. __CountWedges()
	...
	def Init(self):
		...
		self. __dataPos = (0)
		self. __dataSize = (0)
		self. __numSubMeshes = (0)
		self. __posSubMeshes = (0)
		self. __posVerts = (0)
		self. __numVerts = (0)
		self. __posMaterials = (0)
		self. __numTriangles = (0)
		self. __numWedges = (0)

	def ReadMRMFile(self, filename, spaceTransform):
		self. Init()
		...
		try:
			...
			while( not (self. __file.Eof())):
				...
				if(fileBeginning and typ != 0xB100):
					raise RuntimeError(FormatMsg1("File is not a multi-resolution mesh.\nFile name: \"%1\".", self. __file.GetName()))
...
def register(): 
	register_importer("KrxMrmImp", "MRM", "Kerrax Multi-Resolution Mesh") 

def unregister(): 
	unregister_importer("KrxMrmImp") 

if __name__ == "main": 
	register() 

def KrxMrmImp(filename_param, quiet_param = False): 
	QUIET = quiet_param 
	IMPORT_FILE_NAME = filename_param 
	IMPORT_FILE_SIZE = get_file_size(IMPORT_FILE_NAME)
	
	dlgInput = (NewMeshImporterDlgInput())
	dlgInput.SetFileFormat("MRM")
	...
	if( not (QUIET)):
		try:
			...
			inputFile.Close()
			outputFile.Close()
			show_error_box("Kerrax MRM Importer", (ex.args[0] if (len(ex.args) >= 1 and type(ex.args[0]) == str) else str(ex.args)))
			hide_progress_bar()
			return (2)

	loader = (NewMRMFileLoader())
	try:
		if(dlgOutput.GetCurrentSceneMode() == 1):
			reset_scene()

		spaceTransform = (dlgOutput.GetSpaceTransform())
		loader.ReadMRMFile(IMPORT_FILE_NAME, spaceTransform)
